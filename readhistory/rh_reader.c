...
json_t *get_balance_history(MYSQL *conn, uint64_t sid, uint32_t business, uint64_t start_time, uint64_t end_time, size_t offset, size_t limit)
{
    uint32_t hash = sid % HISTORY_HASH_NUM;
    sds sql = sdsempty();
    sql = sdscatprintf(sql, "SELECT `time`, `change`, `balance`, `order`, `comment` FROM `balance_history_%u` WHERE `sid` = %"PRIu64"", hash, sid);

    // business: 0 - all, 1 - update, 2 - trade
    if (business > 0) {
        sql = sdscatprintf(sql, " AND `business` = %d", business);
    }
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        double timestamp = strtod(row[0], NULL);
        json_object_set_new(record, "time", json_real(timestamp));
        json_object_set_new(record, "change", json_string(rstripzero(row[1])));
        json_object_set_new(record, "balance", json_string(rstripzero(row[2])));
        uint64_t order_id = strtoull(row[3], NULL, 0);
        json_object_set_new(record, "order", json_integer(order_id));
        json_object_set_new(record, "comment", json_string(row[4]));
...
json_t *get_order_history(MYSQL *conn, uint64_t sid, const char *symbol, uint64_t start_time, uint64_t end_time, size_t offset, size_t limit)
{
    uint32_t hash = sid % HISTORY_HASH_NUM;
    sds sql = sdsempty();
    sql = sdscatprintf(sql, "SELECT `id`, `side`, `create_time`, `update_time`, `finish_time`, `symbol`, `comment`, `price`, `close_price` "
            ",`lot`, `margin`, `fee`, `swaps`, `profit`, `tp`, `sl`, `external` FROM `order_history_%u` WHERE `sid` = %"PRIu64"", hash, sid);

    if (strlen(symbol) > 0) {
        sql = sdscatprintf(sql, " AND `symbol` = '%s'", symbol);
    }
    ...
    sql = sdscatprintf(sql, " ORDER BY `finish_time` DESC");
    if (offset) {
        sql = sdscatprintf(sql, " LIMIT %zu, %zu", offset, limit);
    } else {
        sql = sdscatprintf(sql, " LIMIT %zu", limit);
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        uint32_t side = atoi(row[1]);
        double create_time = strtod(row[2], NULL);
        double update_time = strtod(row[3], NULL);
        double finish_time = strtod(row[4], NULL);

        json_object_set_new(record, "id", json_integer(id));
        json_object_set_new(record, "side", json_integer(side));
        json_object_set_new(record, "create_time", json_real(create_time));
        json_object_set_new(record, "update_time", json_real(update_time));
        json_object_set_new(record, "finish_time", json_real(finish_time));
        json_object_set_new(record, "symbol", json_string(row[5]));
        json_object_set_new(record, "comment", json_string(row[6]));
        json_object_set_new(record, "price", json_string(rstripzero(row[7])));
        json_object_set_new(record, "close_price", json_string(rstripzero(row[8])));
        json_object_set_new(record, "lot", json_string(rstripzero(row[9])));
        json_object_set_new(record, "margin", json_string(rstripzero(row[10])));
        json_object_set_new(record, "fee", json_string(rstripzero(row[11])));
        json_object_set_new(record, "swaps", json_string(rstripzero(row[12])));
        json_object_set_new(record, "profit", json_string(rstripzero(row[13])));
        json_object_set_new(record, "tp", json_string(rstripzero(row[14])));
        json_object_set_new(record, "sl", json_string(rstripzero(row[15])));

        uint64_t external = strtoull(row[16], NULL, 0);
        json_object_set_new(record, "external", json_integer(external));
