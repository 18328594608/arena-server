...
// balance.history (sid, business, start_time, end_time, offset, limit)
static int on_cmd_balance_history_v2(MYSQL *conn, json_t *params, struct job_reply *rsp)
{
    ...
    // business
    uint32_t business = json_integer_value(json_array_get(params, 1));
    if (business > 2)
        goto invalid_argument;
    ...
    json_t *records = get_balance_history(conn, sid, business, start_time, end_time, offset, limit);
    if (records == NULL) {
        rsp->code = 2;
        rsp->message = sdsnew("internal error");
    }
...
// order.history (sid, symbol, start_time, end_time, offset, limit)
static int on_cmd_order_history_v2(MYSQL *conn, json_t *params, struct job_reply *rsp)
{
    if (json_array_size(params) != 6)
        goto invalid_argument;
    ...
    json_t *records = get_order_history(conn, sid, symbol, start_time, end_time, offset, limit);
    if (records == NULL) {
        rsp->code = 2;
        rsp->message = sdsnew("internal error");
    }
...
static void on_job(nw_job_entry *entry, void *privdata)
{
    ...
    case CMD_BALANCE_HISTORY:
        ret = on_cmd_balance_history_v2(conn, req->params, rsp);
        if (ret < 0) {
            log_error("on_cmd_balance_history fail: %d", ret);
        }
        break;
    case CMD_ORDER_HISTORY:
        ret = on_cmd_order_history_v2(conn, req->params, rsp);
        if (ret < 0) {
