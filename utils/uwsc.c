    ...
    while (1) {
        ...
        *v++ = 0;
        ...
        if (!strcasecmp(k, "Sec-WebSocket-Accept")) {
            ...
            if (strcmp(v, my)) {
                log_error("verify Sec-WebSocket-Accept failed\n");
                return -1;
            }
...
#define SSL_CTX_CHECK                                       \
    do {                                                    \
        if (!ssl_ctx) {                                     \
            ...
            if (!ssl_ctx) {                                 \
                log_error("SSL context init fail\n");   \
                return -1;                                  \
            }                                               \
        }                                                   \
    } while (0)
    ...
    int ping_interval, const char *extra_header)
{
    ...
    if (sock < 0) {
        log_error("tcp_connect failed: %s\n", strerror(errno));
        return -1;
    } else if (sock == 0) {
        log_error("tcp_connect failed: %s\n", gai_strerror(eai));
        return -1;
    ...
    if (ssl) {
        ...
        if (!cl->ssl) {
            log_error("SSL session init fail\n");
            return -1;
        }
#else
        log_error("SSL is not enabled at compile\n");
        uwsc_free(cl);
        return -1;
#endif
    }
