...
static int process_orders_message_v2(json_t *msg)
{
    order_on_update(msg);
    return 0;
}
...
static void on_orders_message(sds message, int64_t offset)
{
    ...
    int ret = process_orders_message_v2(msg);
    if (ret < 0) {
...
static int process_balances_message_v2(json_t *msg)
{
    balance_on_update(msg);
    return 0;
}
...
static void on_balances_message(sds message, int64_t offset)
{
    ...
    int ret = process_balances_message_v2(msg);
    if (ret < 0) {
