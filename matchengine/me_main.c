...
static int init_log(void)
{
    ...
    tick_dlog = dlog_init(settings.tick_log.path, settings.tick_log.shift, settings.tick_log.max, settings.tick_log.num, settings.tick_log.keep);
    if (tick_dlog == NULL)
        return -__LINE__;
...
int main(int argc, char *argv[])
{
    ...
    ret = init_symbol();
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "init symbol fail: %d", ret);
    }
    ret = init_process();
    if (ret < 0) {
    ...
    ret = init_tpsl();
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "init tpsl fail: %d", ret);
    }
    ret = init_stop_out();
    if (ret < 0) {
...
*/
    ...
    ret = init_tick();
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "init tick fail: %d", ret);
    }
