...
//    ret = load_balance(conn, table);
    int ret = load_balance_v2(conn, table);
    if (ret < 0) {
    ...
    log_stderr("load positions from: %s", table);
    ret = load_positions(conn, table);
    if (ret < 0) {
        log_error("load_positions from %s fail: %d", table, ret);
        log_stderr("load_positions from %s fail: %d", table, ret);
        sdsfree(table);
        return -__LINE__;
    }

    sdsclear(table);
    table = sdscatprintf(table, "slice_limit_%ld", timestamp);
    log_stderr("load limits from: %s", table);
    ret = load_limits(conn, table);
    if (ret < 0) {
        log_error("load_limits from %s fail: %d", table, ret);
        log_stderr("load_limits from %s fail: %d", table, ret);
        sdsfree(table);
        return -__LINE__;
    }
...
static int dump_position_to_db(MYSQL *conn, time_t end)
{
    ...
    int ret = dump_positions(conn, table);
    if (ret < 0) {
...
static int dump_limit_to_db(MYSQL *conn, time_t end)
{
    sds table = sdsempty();
    table = sdscatprintf(table, "slice_limit_%ld", end);
    log_info("dump limit to: %s", table);
    int ret = dump_limits(conn, table);
    if (ret < 0) {
        log_error("dump_limit to %s fail: %d", table, ret);
        sdsfree(table);
        return -__LINE__;
    }
    sdsfree(table);
...
int dump_to_db(time_t timestamp)
{
    ...
    int ret = dump_balance_to_db(conn, timestamp);
    if (ret < 0) {
        goto cleanup;
    }
    ...
    ret = dump_limit_to_db(conn, timestamp);
    if (ret < 0) {
        goto cleanup;
    }
...
//    sql = sdscatprintf(sql, "DROP TABLE `slice_order_%ld`", timestamp);
    sql = sdscatprintf(sql, "DROP TABLE `slice_position_%ld`", timestamp);
    log_trace("exec sql: %s", sql);
...
static void on_timer(nw_timer *timer, void *privdata)
{
    time_t now = time(NULL);
    struct tm *lt = localtime(&now);
    // 周二到周六0点要计算隔夜费,不在0点备份
    // 计算完隔夜费再备份
    if (lt->tm_wday > 1 && lt->tm_hour == 0) {
        return;
    }
