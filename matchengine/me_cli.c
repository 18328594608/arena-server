...
static sds on_cmd_balance_list(const char *cmd, int argc, sds *argv)
{
    ...
    while ((entry = dict_next(iter)) != NULL) {
        ...
        if (key->type == BALANCE_TYPE_AVAILABLE) {
            reply = sdscatprintf(reply, "%-10u %-10s %s\n", key->user_id, "available", str);
        } else {
            reply = sdscatprintf(reply, "%-10u %-10s %s\n", key->user_id, "freeze", str);
        }
        free(str);
    }
...
static sds on_cmd_balance_get(const char *cmd, int argc, sds *argv)
{
    ...
    mpd_t *result = balance_get(user_id, BALANCE_TYPE_AVAILABLE);
    if (result) {
        char *str = mpd_to_sci(result, 0);
        reply = sdscatprintf(reply, "%-10u %-10s %s\n", user_id, "available", str);
        free(str);
    }
    result = balance_get(user_id, BALANCE_TYPE_FREEZE);
    if (result) {
        char *str = mpd_to_sci(result, 0);
        reply = sdscatprintf(reply, "%-10u %-10s %s\n", user_id, "freeze", str);
        free(str);
...
static sds on_cmd_balance_summary(const char *cmd, int argc, sds *argv)
{
    ...
    balance_status(total, &available_count, available, &freeze_count, freeze);
    char *total_str = mpd_to_sci(total, 0);
    char *available_str = mpd_to_sci(available, 0);
    char *freeze_str = mpd_to_sci(freeze, 0);
    reply = sdscatprintf(reply, "%-30s %-10zu %-30s %-10zu %-30s\n",
