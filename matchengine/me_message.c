...
int push_balance_message_v2(double t, uint64_t sid, mpd_t *change, mpd_t *balance, const char *comment)
{
    json_t *message = json_array();
    json_array_append_new(message, json_real(t));
    json_array_append_new(message, json_integer(sid));
    json_array_append_mpd(message, change);
    json_array_append_mpd(message, balance);
    json_array_append_new(message, json_string(comment));
...
{
    json_t *message = json_array();
    json_array_append_new(message, json_integer(event));

    json_array_append_new(message, json_integer(order->id));
    json_array_append_new(message, json_integer(order->sid));
    json_array_append_new(message, json_integer(order->side));
    json_array_append_new(message, json_string(order->symbol));
    json_array_append_new(message, json_real(order->create_time));
    json_array_append_new(message, json_real(order->update_time));
    json_array_append_new(message, json_real(order->finish_time));
    json_array_append_new(message, json_integer(order->expire_time));

    json_array_append_mpd(message, order->lot);
    json_array_append_mpd(message, order->price);
    json_array_append_mpd(message, order->close_price);
    json_array_append_mpd(message, order->fee);
    json_array_append_mpd(message, order->swaps);
    json_array_append_mpd(message, order->profit);
    json_array_append_mpd(message, order->tp);
    json_array_append_mpd(message, order->sl);
    json_array_append_new(message, json_string(order->comment));
